...
- [ ] Monorepo scaffold: root `package.json` + `pnpm-workspace.yaml` (`apps/*`), `.gitignore`, `.env.example`, `README.md`
- [ ] Create the three app folders: `apps/web`, `apps/api`, `apps/tracker`
- [ ] Backend deps: `apps/api/pyproject.toml` + `.python-version` (3.12); `uv sync` creates `.venv/`
- [ ] Frontend deps: Next.js app in `apps/web`; `pnpm install`
- [ ] Minimal API: app factory `app/main.py`, settings `app/config.py`, `GET /health`
- [ ] Minimal web: Next.js runs and shows a placeholder page
- [ ] One passing test (`apps/api/tests/test_health.py`) and `ruff` clean
- [ ] Verify locally: `uv run uvicorn ...`, `pnpm --filter web dev`, `uv run pytest` all work
- [ ] Init git, create the GitHub repo, and push

### Phase 1 — Database & auth system
**Goal:** a user can sign up, log in, and reach a protected dashboard.
- [ ] Local services reachable (Postgres + ClickHouse + Redis) via their env-var connection strings
- [ ] DB clients: `db/postgres.py`, `db/clickhouse.py`, `db/redis.py`
- [ ] Postgres schema + Alembic baseline: `accounts`, `sites`, `subscriptions` (see Core data model)
- [ ] Password hashing util (argon2/bcrypt) in `core/security.py`
- [ ] `POST /auth/signup` (`routers/auth.py` -> `services/auth.py`): create account, store `password_hash`
- [ ] `POST /auth/login`: verify password, issue access + refresh JWT
- [ ] `POST /auth/refresh`: rotate the access token
- [ ] JWT verification dependency `require_user` in `core/security.py`; tenant scoping (`user_id -> account`)
- [ ] Rate-limit `/auth/login`
- [ ] Frontend: custom `sign-in/` + `sign-up/` pages calling `/auth/*`; store token in `lib/` api client; guard `(dashboard)` routes
- [ ] Tests: signup, login, token verify, protected route rejects missing/invalid token
...
_Last updated: keep this date current whenever the stack or rules change._
