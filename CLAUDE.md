...
- **Pricing is usage-based (graduated metered)** — billed by monthly pageviews summed account-wide across all sites, via Stripe. First **1,000 views/month free**, then $0.99/1k to 10k, $0.10/1k to 100k, $0.05/1k to 1M, $0.03/1k beyond (the ONE source of truth is `apps/web/lib/pricing.ts`). A **7-day free trial that starts at upgrade** (not signup), **once per account**. A free account over 1k is locked out of the *dashboard* (blocking paywall + 402), but **ingestion is never gated** (§9). Max **5 sites per account**.
...
```
events          site_id, ts (UTC), event_type ("pageview"|"custom"), name (custom
                event name), path, referrer, source, utm_source, utm_medium,
                utm_campaign, country, region, city, device, browser, os, language,
                visitor_hash, screen_w
```
`event_type`/`name` (Phase 15) tag a custom event fired via the tracker's
`flowly('event', name)`. Custom events are **stored but never metered or shown in
the live counter** (they aren't pageviews — §1 bills by pageview); their reports
(`/events`, `/goals/*`) are **paid-only** (free → 402), while `/collect` still
ingests them for everyone (§9). Conversion goals live in Postgres:
```
goals           id, site_id (FK), name, kind ("pageview"|"custom"), target, created_at
                (unique(site_id, kind, target))
```

**ClickHouse schema migrations** (Alembic is Postgres-only). ClickHouse `events`
changes live in `app/db/ch_migrations.py` as an **ordered, idempotent** list of
`ALTER TABLE events ADD COLUMN IF NOT EXISTS …` statements. Rules: append new
migrations to the end (never edit/reorder a shipped one), keep every statement
idempotent (idempotency *is* the versioning — there's no version table), and keep
`db/clickhouse.py::CREATE_EVENTS_TABLE` in sync so fresh installs get the columns
at create time. Apply with `uv run python -m app.db.clickhouse --migrate` (runs
`--init` first, then the list); safe to re-run.
...
| LLM / support chatbot | **Anthropic SDK** (`anthropic`, async) — small/cheap model (Claude Haiku) for the F7 assistant's AI fallback only; key-gated + guardrailed | any other LLM provider, home-grown API calls |
...
| Postgres | `DATABASE_URL` (a bare `postgres://`/`postgresql://` DSN from a managed provider is auto-upgraded to the `+asyncpg` driver in `config.py`) |
| ClickHouse | `CLICKHOUSE_HOST`, `CLICKHOUSE_USER`, `CLICKHOUSE_PASSWORD`, `CLICKHOUSE_DB`, `CLICKHOUSE_SECURE` (TLS — `true` for ClickHouse Cloud; port then defaults to 8443), `CLICKHOUSE_PORT` (blank → clickhouse-connect default) |
| Redis | `REDIS_URL` |
...
| Stripe (metered, F14) | `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_METERED` (the single graduated-tiered usage Price), `STRIPE_METER_EVENT` (Billing Meter event name usage is pushed under; default `pageviews`) |
| Uptime (F12) | `UPTIME_ENABLED` (master switch — off by default; nothing pings customer sites until an operator turns it on), `UPTIME_CHECK_TIMEOUT` (per-check seconds), `UPTIME_FAIL_THRESHOLD` (consecutive fails before an incident opens + the owner is alerted), `UPTIME_MAX_REDIRECTS` (each hop re-screened by the SSRF guard) |
| Search Console (F13) | reuses `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` (sign-in creds) with the `webmasters.readonly` scope — add `{API_BASE_URL}/searchconsole/callback` to the Google client's authorized redirect URIs; `GSC_SYNC_DAYS` (trailing days re-pulled per daily sync), `GSC_ROW_LIMIT` (rows/day/site from the Search Analytics API) |
| Email | `EMAIL_API_KEY`, `CONTACT_EMAIL` (public contact-form inbox; blank → `EMAIL_FROM`) |
| Assistant (F7) | `ANTHROPIC_API_KEY` (support-chatbot AI fallback; blank → hardcoded FAQ + contact fallback, no API call) |
...
uv run python -m app.workers.uptime       # uptime pinger + down/up alerts (cron: every ~5 min)
uv run python -m app.workers.searchconsole # Search Console daily sync (cron: daily)
uv run python -m app.workers.usage_report # push metered usage to Stripe (cron: hourly)
uv run alembic upgrade head               # apply Postgres migrations
uv run alembic revision --autogenerate -m "msg"   # create a migration
uv run python -m app.db.clickhouse --migrate      # apply ClickHouse events migrations (idempotent)
...
- [x] Channel classifier (`services/channels.py`): bucket `referrer` host → direct / search / social / **AI platforms** / referral; `/stats/channels` (5-way split) + `/stats/channels/{channel}` per-host drill-down for search/social/ai. Host lists are the single source of truth shared by `classify()` (unit-tested) and the SQL `multiIf` (AI checked before search)
- [x] Screen-size / viewport breakdown: `multiIf` width buckets over existing `screen_w` (≤640 Mobile / ≤1024 Tablet / ≤1440 Laptop / else Desktop; 0 → Unknown) on `/stats/audience?dimension=screen`
- [x] Time-of-day / day-of-week heatmap: `/stats/heatmap` — **§4 exception: aggregate in the viewer's IANA tz** via `toHour(ts, {tz})` / `toDayOfWeek(ts, 0, {tz})` (NB: dow's tz is the 3rd arg, 2nd is week_mode), 7×24 grid zero-filled in Python; `components/reports/heatmap-card.tsx` panel in Overview
- [x] Custom date range picker (frontend-only): native date-input popover (`components/dashboard/custom-range.tsx`, dependency-free — no shadcn Calendar/react-day-picker) writing a custom `[from,to)` into the range context; server already parses arbitrary ranges via `core/timerange.py::stats_range`
- [x] Dashboard-wide click-to-filter + multi-filter combos: allowlisted `_FILTER_COLUMNS` filter dict threaded into every `(sql, params)` builder + inside `_SESSIONIZED_CTE` (**server-parameterized, namespaced `f_*` params, §9**); shared `core/statsfilters.py::FilterDep`; `components/layout/filter-context.tsx` + `active-filters.tsx` chip bar + `onSelect` on breakdown/pages cards; every chart/table re-filters
- [x] Page detail view: click a page row → `path` filter applied → all reports re-slice (falls out of the filter work)
- [x] Top-10 pages ranked by traffic and by engagement (`sort=engagement`: per-page bounce % + avg time-on-page via `leadInFrame` over `_SESSIONIZED_CTE`; traffic/engagement toggle on the Pages report)
...
- [x] **ClickHouse migration convention** (Alembic is Postgres-only, §3): `app/db/ch_migrations.py` = ordered, idempotent `ADD COLUMN IF NOT EXISTS` statements + `run_migrations()`; `uv run python -m app.db.clickhouse --migrate` (runs `--init` then the list). `CREATE_EVENTS_TABLE` kept in sync (fresh installs get columns at create; existing get them from ALTERs). Documented in §3.
- [x] City breakdown: `geo.lookup` now returns `(country, region, city)` (fail-open), threaded through ingest→stream→batch_writer→`city` column; `/stats/audience?dimension=city`. **Paid-tier report** — `billing.require_dimension_access` (→ 402 `UpgradeRequiredError`) gates it at read time on all three paths (authed stats, public share, CSV export); free UI shows an upgrade `EmptyState`.
- [x] Language breakdown: `navigator.language` re-added to the tracker (rebuilt **1.1 KB** < 2 KB), passed through `/collect` (`models/events.py`, normalized lowercase), new `language LowCardinality(String)` column, `/stats/audience?dimension=language` (free).
- [x] Live-by-country panel: `useLiveTraffic` aggregates the live feed's existing `country` into `currentCountries`; `LiveCountries` panel (emoji flags + live counts + share bars) on the live page — no backend change. (A full geographic choropleth needs a bundled world-map asset; deferred — see build note.)

### Phase 12 — Uptime monitoring & down alerts
**Goal:** ping each site on a schedule; email the owner when it goes down / recovers.
- [x] Pinger worker (`workers/uptime.py`, run-once + cron like digest/retention, gated on `UPTIME_ENABLED`): HTTP check per site domain (`services/uptime.py::check_with_retry`) with timeout + an in-run retry (blip filter) + a consecutive-failure threshold before alerting (retry-before-alarm); `uptime_monitors` (current state + fail streak) + `uptime_incidents` (ledger) in Postgres (migration `a1b2c3d4e5f6`). Network checks fan out with bounded concurrency; state writes stay sequential on the one AsyncSession
- [x] **SSRF guard** (`services/uptime.py::_screen_host`): `domain` is an unverified, user-supplied label (Phase 6) — every request (and every redirect hop) resolves the host and refuses any non-global address (`not ip.is_global` covers loopback/private/link-local incl. 169.254.169.254/reserved/multicast; IPv4-mapped IPv6 unwrapped first); redirects followed manually (`follow_redirects=False`, re-screened per hop — the classic bypass), capped at `UPTIME_MAX_REDIRECTS`, GET/HEAD only. A "blocked" result is recorded but never opens an incident/alert (misconfig, not an outage)
- [x] Down / recovered email alerts (`services/email.py::send_uptime_{down,up}_email`): **transactional — direct `send_email`, NEVER through `services/notifications.py` (the marketing opt-out gate)**; alert once per incident via the open incident's `notified_down`/`notified_up` flags (best-effort — a failed send leaves the flag false to retry), not per failed ping. Only verified owners are emailed
- [x] Dashboard status view: `GET /sites/{site_id}/uptime` (authed, ownership-scoped via `owned_site`) → `UptimeStatusOut`; `hooks/useSites.ts::useSiteUptime` + `components/uptime/uptime-panel.tsx` (green/red up/down `Badge` + recent-incidents list) on the site detail view
- [x] Tests (`tests/test_uptime.py`): down→alert-once (opens at threshold, silent after), recover→notice (resolve + one email), flapping doesn't spam (single blip below threshold → no incident), no alert through the marketing gate (opted-out owner still alerted; `notifications.send_marketing_email` asserted never called), SSRF targets refused (loopback/private/link-local/metadata/IPv4-mapped + redirect-to-internal), unverified owner not emailed, blocked ≠ incident, endpoint ownership 404
...
- [x] GSC connect (`services/searchconsole.py` + `services/gscapi.py` + `routers/searchconsole.py`): a **separate offline-access flow** (sign-in stays `access_type=online`) reusing the Google client creds with the `webmasters.readonly` scope + `access_type=offline`&`prompt=consent`. Authed `POST /searchconsole/{site_id}/connect` returns the consent URL (state binds `(account,site)` in Redis); public `GET /searchconsole/callback` exchanges the code, auto-matches the site's domain to a verified GSC property (prefer `sc-domain:`), and stores the **refresh token** (new `search_console_connections` table, Alembic `b2c3d4e5f6a7`; token **never logged or returned**, §9)
- [x] Sync worker (`workers/searchconsole.py`, run-once + cron daily): `sync_all` → per connection refresh the access token, pull Search Analytics (query, page, clicks, impressions, position) into `search_metrics`; **idempotent per (site, day)** via delete-then-reinsert; best-effort per site (one failure never aborts the sweep). CTR is derived at read time, not stored
- [x] Keyword performance report: `GET /searchconsole/{site_id}/keywords` — top queries by clicks, impression-weighted average position, derived CTR, date-ranged (shared `stats_range`)
- [x] Page-level search performance: `GET /searchconsole/{site_id}/pages` — pages ranked by search clicks
- [x] Opportunity-keyword recommendations: `GET /searchconsole/{site_id}/opportunities` — HAVING weighted position 5–20, ranked by impressions ("just off page one"), **no paid third-party keyword API** (derived from the same synced data)
- [x] Note held: the JS snippet cannot see search keywords (stripped from referrers) — GSC is the only honest source. **Metrics stored in Postgres** (not ClickHouse): external, low-volume, daily-aggregated, and per-(site,day) idempotency is a clean Postgres transaction. Reports/connect are authed + ownership-scoped (`owned_site`, 404-before-query, §9); frontend `/search-console/{keywords,pages,opportunities}` pages with a connect empty-state (`hooks/useSearchConsole.ts`, `components/searchconsole/search-report.tsx`, new nav group)
...
- [x] **Decommission the flat-plan model (replaces Phase 7's pricing, not its plumbing):** removed the 4 flat `STRIPE_PRICE_*` ids + `PLAN_QUOTAS` + `HARD_CEILING_MULTIPLE`; kept and reused Phase 7's Checkout/Portal/webhook/idempotency/metering infrastructure. `effective_plan` collapses to two states: **free** (no active sub) vs **metered** (active/past-due/in-window-trialing sub). No live flat-plan subscribers (pre-launch, confirmed) → no migration needed
- [x] **Re-position the 7-day trial to the paywall:** signup no longer sets `trial_ends_at` (accounts start `free`/`free`); the trial starts at Checkout (`subscription_data.trial_period_days=7`), granted only when `_has_used_trial` is false. **One trial per account** — the trialing webhook stamps `trial_ends_at`, so a re-subscribe gets none; an unconverted trial lapses to free → re-locks while over 1k. `trial_will_end` nudge kept; §1 updated
- [x] Stripe metered billing (`services/billing.py`): ONE graduated-tiered metered Price (`STRIPE_PRICE_METERED`); `report_usage_to_stripe` pushes the **delta** since last push (Billing Meter events are additive) from the Redis counter, tracked by a `usage_reported:{account}:{YYYYMM}` marker so re-runs never double-count; new `workers/usage_report.py` (cron hourly). **Webhooks stay the only writer of entitlement**
- [x] **Free-limit paywall:** `usage_summary` emits `status: locked` when a free account passes 1k; `components/PaywallGate.tsx` mounts a **non-dismissible** `PaywallModal` over the dashboard; server enforces via `billing.ensure_not_locked` → typed 402 `AccountLockedError` (machine `code: account_locked`) on all stats routes (router-level dep) + the live WS. CTA → metered Checkout → webhook flips entitlement → unlocks. **`/collect` never gates (§9)** — asserted by `test_collect_still_ingests_while_locked`
- [x] **Close the side doors while locked:** public share data routes 402 via `unlocked_shared_site` (meta stays open for a paused state); CSV export follows the router-level stats lock; the weekly digest skips locked accounts (`workers/digest.py`)
- [x] Free-tier UX before the wall: `UsageBanner` — warning at ≥80% of 1k, locked at >1k; paying accounts see a running month-to-date bill estimate (client-side `lib/pricing.ts` over the Redis counter)
- [x] 5-site limit (`services/sites.py::create_site`): count before insert → typed `SiteLimitError` (403) with an upgrade-path message; enforced in the service (§3); frontend hides "Add site" at 5 (`MAX_SITES_PER_ACCOUNT` / `MAX_SITES`)
- [x] Re-map plan-keyed logic: retention windows (`config.RETENTION_DAYS` → free 30d / **metered 365d**), `show_badge` (`effective_plan == free`)
- [x] Updated §1 (pricing facts + trial-at-upgrade), §6 (Stripe env vars), `.env.example`, §7 (usage-report cron), pricing page (metered, F5/F1)
- [x] **OPEN (growth call, cheap to change later):** free tier stays at **1k** for now (confirmed) — `config.FREE_MONTHLY_VIEWS` / `lib/pricing.ts::FREE_MONTHLY_VIEWS`
- [x] Tests: graduated tier math at exact boundaries (Vitest `lib/pricing.test.ts`, 1k/10k/100k/1M), free-tier no-charge, paywall lock at >1k free + unlock via webhook (never via redirect), trial-once-per-account (second checkout gets no trial), trial-expiry re-locks, `/collect` still ingests while locked, side doors (share/export/digest) follow the lock, site-limit at 5 (not at 4), webhook idempotency, retention free/metered mapping (`test_paywall.py`, `test_billing_service.py`, `test_billing_router.py`, `test_sites_service.py`, `test_digest.py`, `test_retention.py`)

### Phase 15 — Premium (DEFER until users ask)
**Goal:** upsell features — build only when paying users request them.
- [x] **Custom events + conversion goals** (first premium feature built) — tracker `flowly('event', name)` (dist rebuilt **1.4 KB** < 2 KB) → `/collect` (`models/events.py` gains `event_type`/`name`) → ClickHouse `event_type`/`name` columns (`ch_migrations.py` 0003/0004, `CREATE_EVENTS_TABLE` + `EVENT_COLUMNS` + batch-writer coerce in lockstep). **Stored-only:** a custom event is NOT metered (§1 bills by pageview — `ingest_event` gates `meter_pageview` on `event_type=='pageview'`) and NOT in the live counter. Reports are **paid-gated** (`billing.require_premium` → 402): `GET /events` (top custom events) + goals CRUD/conversions (`services/goals.py`, `routers/goals.py`, `goals` table via Alembic `d4e5f6a7b8c9`). Conversion = distinct converting visitors ÷ distinct visitors (`stats.build_goal_conversions`, target server-parameterized §9). Ownership-scoped (`owned_site` → 404). Web: `/goals` page (`components/goals/goals-report.tsx`, `hooks/useGoals.ts`, nav "Goals & events" under Behavior; free → upgrade `EmptyState`). Tests: `test_goals.py` (gate 402 / ownership 404 / CRUD / dup 409 / conversion), `test_ingest.py` (custom stored, not metered, not live), `test_stats_builders.py` (events + goal SQL, target bound not interpolated)
- [ ] Custom dashboards
...
_Last updated: 2026-07-07 — **Phase 15 (Premium) — first feature built: custom events + conversion goals.** The tracker gained a public `flowly('event', name)` API (drains a pre-load `flowly.q` queue; `dist/script.js` rebuilt **1.4 KB** < 2 KB); `/collect` (`models/events.py`) gained `event_type` ("pageview"|"custom", forgiving default) + `name`, threaded IP-free through `ingest.build_stream_row` → Redis stream → `batch_writer._coerce_row` → two new ClickHouse columns (`event_type LowCardinality DEFAULT 'pageview'`, `name String`; `ch_migrations.py` 0003/0004 + `CREATE_EVENTS_TABLE`/`EVENT_COLUMNS` in lockstep — applied + verified on dev ClickHouse). **Billing correctness (§9):** a custom event is NOT a pageview — `ingest_event` gates BOTH `meter_pageview` and the live fan-out on `event_type=='pageview'`, so custom events are **stored-only** (verified live: usage stayed 1 after 1 pageview + 2 custom events; live presence set stayed empty). Reports are **paid-only** — new `billing.require_premium` raises 402 `UpgradeRequiredError` for a free account on every route: `GET /events` (top custom events, `stats.build_events`) and conversion goals (`services/goals.py` CRUD + `stats.build_goal_conversions` = distinct converting visitors ÷ distinct visitors, target server-parameterized §9). Goals live in a new Postgres `goals` table (Alembic `d4e5f6a7b8c9`, additive; unique(site_id, kind, target) → dup = clean 409). `routers/goals.py` is authed + ownership-scoped (`owned_site` → 404 before any work) + entitlement-gated. **Web:** `hooks/useGoals.ts` + `components/goals/goals-report.tsx` (custom-events table + goals list with per-goal conversion rate + add/delete form; free → upgrade `EmptyState`) + `/goals` page under the Behavior nav group ("Goals & events"). No new dep, no new env var. **333 backend tests pass** (+17: `test_goals.py` gate-402/ownership-404/CRUD/dup-409/conversion-rate; `test_ingest.py` custom stored-not-metered-not-live; `test_stats_builders.py` events + goal SQL, target bound not interpolated), ruff clean, single Alembic head; web lint + tsc + build green (`/goals` route added), 9 Vitest pass, grep gate holds. **Live-verified end-to-end** on the running stack: signup→site→fire pageview + 2 custom events (all 202)→flush→ClickHouse shows `custom/signup`×2 + `pageview`×1; free account → /events + /goals 402; after flipping to metered → events report (signup: 2/2), goal create, conversion 2/2 = 1.0, dup 409, unowned site 404. **Manual before deploy:** run `uv run python -m app.db.clickhouse --migrate` + `alembic upgrade head` in prod; ship the rebuilt `dist/script.js`. **Deferred within Phase 15** (build only when asked): custom-event *properties*/revenue (a PII surface, §9 — name-only for now), funnels/segments (build on this event foundation), public API, Slack, white-label, team seats. **Retention reports stay parked** (needs cross-day identity — conflicts with the daily-rotating visitor_hash, §2/§9). **Phase 14 (Pricing v2 — graduated metered billing + paywall + 5-site limit) built.** Replaced the Phase 7 flat plans with usage-priced billing, reusing all of Phase 7's Stripe/webhook/metering plumbing. **Entitlement collapses to two states** in `billing.effective_plan`: **free** (no active sub — the signup default; `plan="free"`/`status="free"`) vs **metered** (active/past-due/in-window-trialing sub). `config.py` dropped `PLAN_QUOTAS`/`HARD_CEILING_MULTIPLE`/4 flat `STRIPE_PRICE_*`; added `FREE_MONTHLY_VIEWS=1000`, `MAX_SITES_PER_ACCOUNT=5`, `stripe_price_metered`/`stripe_meter_event`, `RETENTION_DAYS={free:30, metered:365}`. **The free-over-limit paywall:** `usage_summary` emits `status: locked` when a free account passes 1k (account-wide `usage:{account_id}:{YYYYMM}` counter, unchanged); `billing.ensure_not_locked` raises a typed **402 `AccountLockedError`** (machine `code: account_locked`, new `AppError.code` field surfaced by the handler + web `ApiError.code`) — enforced by a **router-level dep on all `/stats/*` routes (incl. CSV export)** and inside the **live WS** `_authorize`; web `components/PaywallGate.tsx` mounts a **non-dismissible** `PaywallModal` when locked. **`/collect` is NEVER gated (§9)** — `meter_pageview` has no lock check; `test_collect_still_ingests_while_locked` asserts 202 while locked. **Side doors follow the lock** (confirmed default): public share **data** routes 402 via `unlocked_shared_site` (meta stays open for a paused state), export via the stats dep, and the weekly **digest skips** locked accounts. **Trial repositioned to upgrade:** signup no longer sets `trial_ends_at`; Checkout adds `trial_period_days=7` only when `_has_used_trial` is false (**one trial per account** — the trialing webhook stamps `trial_ends_at`, so a re-subscribe gets none; an unconverted trial lapses to free and re-locks over 1k). **Stripe metered:** ONE graduated-tiered metered Price; `report_usage_to_stripe` pushes the **delta** since the last push (meter events are additive) tracked by a `usage_reported:{account}:{YYYYMM}` marker so re-runs never double-count → new `workers/usage_report.py` (cron hourly). **Webhooks stay the only writer of entitlement.** 5-site cap in `services/sites.py::create_site` → typed **403 `SiteLimitError`** (enforced in the service, §3); web hides "Add site" at 5. `show_badge` = `effective_plan==free` (unchanged expression, new value). **Pricing math single source stays `apps/web/lib/pricing.ts`** (graduated integer-cents; boundary bills 10k→$8.91/100k→$17.91/1M→$62.91 pinned by `lib/pricing.test.ts`) — the backend only pushes usage quantity, Stripe's Price does the math. **No live flat-plan subscribers (pre-launch, confirmed) → no migration/notification** (plan/status are strings, `trial_ends_at` already exists). **Post-review hardening (2 pre-go-live billing fixes):** (1) `create_checkout_session` now blocks any **entitled** account (`effective_plan == metered`, incl. an in-window **trialing** account) — the old active/past_due-only guard let a trialing user re-run Checkout and mint a second live subscription (double bill). (2) The usage-push "already reported" high-water mark moved from Redis to a **durable Postgres marker** (`subscriptions.metered_usage_reported`/`metered_usage_period`, migration `c3d4e5f6a7b8`, additive/batch-mode, round-tripped by `test_migration`) — an evicted Redis marker would have let the delta-push re-push a whole month and Stripe's additive meter would **double-bill** (§9); with the durable marker the worst case on a Redis flush is *under*-billing (counter < marker → delta ≤ 0 → skip), never over. §1 (pricing facts + trial-at-upgrade), §6 (Stripe env vars), `.env.example`, §7 (usage-report cron) updated. **314 backend tests pass** (rewritten `test_billing_service`/`test_billing_router` for the metered model; new `test_paywall.py` — stats/export 402 when locked, metered never locked, public share follows lock + meta open, `/collect` ingests while locked; `test_sites_service` site-limit at 5-not-4; `test_digest` skips-locked; `test_retention` free/metered mapping; `test_billing_service` lock/trial-once/delta-usage-push), ruff clean; web lint + tsc + build green (routes unchanged), 9 Vitest pass. **Manual before deploy:** run `alembic upgrade head` (migration `c3d4e5f6a7b8`), create the graduated metered Price + Billing Meter in Stripe, set `STRIPE_PRICE_METERED`/`STRIPE_METER_EVENT`, wire `app.workers.usage_report` to an hourly scheduler, and run a live Checkout→webhook→meter smoke (`stripe listen`). **Phase 13 (Search Console integration — keywords & SEO) built.** Shows the customer's own GSC search keywords, average position, and opportunity keywords. **OAuth:** a **separate offline-access flow** (sign-in stays `access_type=online`; a shared flow would force a refresh token onto every login), reusing `GOOGLE_CLIENT_ID/SECRET` with the `webmasters.readonly` scope + `access_type=offline`&`prompt=consent`. Authed `POST /searchconsole/{site_id}/connect` returns the consent URL and binds a Redis `state → (account, site)` (the bearer token can't ride Google's top-level redirect); public `GET /searchconsole/callback` exchanges the code, auto-matches the site's domain to a verified property (prefer `sc-domain:`, then https/www/http variants — no picker UI), and stores the **refresh token**. **Two new Postgres tables** (migration `b2c3d4e5f6a7`, additive, applied + downgrade-verified on dev Postgres + SQLite migration test): `search_console_connections` (site↔property + refresh_token + `last_synced_at`, 1:1 per site) and `search_metrics` (date/query/page/clicks/impressions/position). **Metrics live in Postgres, not ClickHouse** — GSC data is external, low-volume, daily-aggregated, and its per-(site,day) idempotency is a trivial delete-then-reinsert in one Postgres transaction (append-only ClickHouse would need mutations); scale caveat noted. **`services/gscapi.py`** = pure Google HTTP layer (authorize URL, code→refresh-token exchange, access-token refresh, list properties, `query_search_analytics`) — plain httpx, **no google-api-python-client dep**; **`services/searchconsole.py`** = connect state + store/link/disconnect + `sync_site`/`sync_all` (trailing `GSC_SYNC_DAYS`, skip the freshest 2 GSC-lag days, delete-reinsert per day) + the three report builders (impression-weighted avg position, derived CTR; opportunities = HAVING position 5–20 ranked by impressions). `workers/searchconsole.py` (run-once + cron daily) syncs all connections best-effort (one failure never aborts; **tokens never logged**, §9). **`routers/searchconsole.py`**: authed ownership-scoped `connect`/`connection`/`sync`/`disconnect`/`keywords`/`pages`/`opportunities` (`owned_site` → 404 before any work) + the public `callback`; the connection endpoint **never returns the refresh token** (asserted in tests). **Reused** `GOOGLE_CLIENT_*` + `stats_range`; **2 new env vars** (`GSC_SYNC_DAYS`/`GSC_ROW_LIMIT` in `config.py` + `.env.example` + §6), new §7 worker cron. **Frontend:** `hooks/useSearchConsole.ts` (connection/connect/sync/disconnect + report queries), `components/searchconsole/search-report.tsx` (connect empty-state → GSC consent; table with clicks/impressions/CTR/position; sync-now + disconnect + `?gsc=connected|error` toast), three `app/(dashboard)/search-console/{keywords,pages,opportunities}` pages, new "Search Console" nav group. 296 backend tests pass (new `tests/test_searchconsole.py`, 13 — property auto-match, connect-state round-trip, sync idempotency per (site,day), best-effort survives a failure, keyword aggregation + weighted position + CTR, opportunity position-band filter, ownership 404, refresh-token-never-returned, connect→callback stores connection, disconnect wipes, worker run), ruff clean; web lint + tsc + build green (+3 routes: `/search-console/keywords,pages,opportunities`), 9 Vitest pass, grep gate holds. **Manual before deploy:** register `{API_BASE_URL}/searchconsole/callback` in the Google OAuth client's authorized redirect URIs; live-verification needs real Google creds + a verified GSC property (tests mock the GSC API). **Deferred/decisions:** property auto-match by domain (a manual picker if a user has multiple properties is future); refresh-token encryption-at-rest recommended (stored plaintext today, guarded by never-log/never-return); metrics→ClickHouse if a customer's GSC volume grows large. **Phase 12 (uptime monitoring & down alerts) built.** A cron-driven pinger checks each registered site and emails the owner when it goes down / recovers. **New Postgres schema** (migration `a1b2c3d4e5f6`, additive — two new tables, applied + downgrade-verified on dev Postgres and via the SQLite migration test): `uptime_monitors` (1:1 per site — `status`/`fail_streak`/`last_status_code`/`last_checked_at`, the cross-run memory that powers retry-before-alarm) + `uptime_incidents` (ledger; the **open** row `resolved_at IS NULL` is the alert-idempotency key). **`services/uptime.py`** holds all logic: (1) **the check** — `check_domain`/`check_with_retry` GET/HEAD `https://{domain}` via existing httpx, in-run retry as a blip filter; "down" = timeout / connect / DNS / 5xx, any completed <500 (incl. 3xx/4xx) = up; (2) **the SSRF guard** `_screen_host` — `domain` is an *unverified* Phase-6 label, so every request AND every redirect hop resolves the host (`_resolve`, split out for test injection) and refuses any `not ip.is_global` address (loopback/private/link-local incl. `169.254.169.254`/reserved/multicast; IPv4-mapped IPv6 unwrapped first); redirects followed manually (`follow_redirects=False`, re-screened per hop — the classic bypass), capped at `UPTIME_MAX_REDIRECTS`, GET/HEAD only; a "blocked" internal target is recorded but never alerts (misconfig ≠ outage); (3) **state + alerts** `process_result` — a per-site `UptimeMonitor` counts consecutive failures, an incident opens only once `fail_streak >= UPTIME_FAIL_THRESHOLD` (retry-before-alarm — a lone failed check never pages), the owner is emailed **once** on open + once on recovery (guarded by `notified_down`/`notified_up`, best-effort so a failed send retries next run), only if verified. **Alerts are transactional** (`services/email.py::send_uptime_{down,up}_email` → direct `send_email`, **never** `services/notifications.py`'s marketing opt-out gate — an opted-out owner still gets down alerts; asserted in tests). `sweep()` fans out the network checks with bounded concurrency (they touch no DB) then applies state sequentially on the single AsyncSession (not concurrency-safe). **`workers/uptime.py`** (run-once + external cron ~5 min, gated on `UPTIME_ENABLED`, off by default so nothing pings customer sites until an operator enables it) mirrors digest/retention. **Read surface:** `GET /sites/{site_id}/uptime` (authed, ownership-scoped via the existing `owned_site` dep → 404 before any query) → `UptimeStatusOut` (status + `last_checked_at` + recent incidents); web `hooks/useSites.ts::useSiteUptime` + `components/uptime/uptime-panel.tsx` (green/red up/down `Badge` on the F0 semantic tokens + recent-incidents list, UTC localized at display) on the site detail view; new `lib/format.ts::formatDateTime`. **No new dependency** (SSRF guard is stdlib `ipaddress`/`socket`; checks ride the existing httpx), **4 new env vars** (`UPTIME_ENABLED`/`UPTIME_CHECK_TIMEOUT`/`UPTIME_FAIL_THRESHOLD`/`UPTIME_MAX_REDIRECTS` in `config.py` + `.env.example` + §6), new §7 worker cron command. 283 backend tests pass (new `tests/test_uptime.py`: down→alert-once, recover→notice, flapping-no-spam, no-marketing-gate, SSRF incl. redirect-to-internal + IPv4-mapped, unverified-not-emailed, blocked≠incident, ownership 404), ruff clean; web lint + tsc + build green (routes unchanged, `/sites` gains the panel), 9 Vitest pass, grep gate holds. **Manual before deploy:** set `UPTIME_ENABLED=true` + wire `app.workers.uptime` to a ~5-min scheduler; a real `EMAIL_API_KEY` (Resend) for live alerts (dev stub logs otherwise). **Known limitation (documented, deferred):** the SSRF guard resolves-then-connects, leaving a small DNS-rebinding TOCTOU window — acceptable at current scale; harden to IP-pinning if this becomes a high-volume internet-facing surface. **Phase 11 (city, language, live-by-country + first ClickHouse migration) built** — the first change to the `events` schema, so it establishes the **ClickHouse migration convention**: `app/db/ch_migrations.py` holds an ordered, idempotent list of `ALTER TABLE events ADD COLUMN IF NOT EXISTS …` statements + `run_migrations()`, run via `uv run python -m app.db.clickhouse --migrate` (init-then-migrate, safe to re-run); `CREATE_EVENTS_TABLE` kept in sync so fresh installs get the columns at create time; documented in §3 + §7. Two new columns: `city String`, `language LowCardinality(String)` (applied + verified on dev ClickHouse; migration is idempotent on re-run). **Enrichment chain:** `geo.lookup` now returns `(country, region, city)` (still fail-open → `("","","")`) → `ingest.build_stream_row` → Redis stream → `batch_writer._coerce_row` (both new fields default `""` so pre-migration stream entries still coerce) → the named-column insert. **Language** re-added to the tracker (`navigator.language`; `dist/script.js` rebuilt **1.1 KB** < 2 KB) → `models/events.py` (`extra="ignore"` already tolerated it; now a real field, normalized lowercase) → `language` column. **City is the first paid-tier report:** new `billing.require_dimension_access` (→ 402 `UpgradeRequiredError`, new in `core/exceptions.py`) gates `dimension=city` at read time on ALL three paths — authed `/stats/audience`, public `/public/{token}/audience` (uses the share owner's plan), and CSV `/stats/export` (`dataset=audience`) — so the gate isn't UI-only (§9). `_AUDIENCE_COLUMNS` gained `city`/`language`; the audience `dimension` Literals updated on all three routers. **Frontend:** `AudienceDimension` gained `city`/`language`; `/geo/cities` + `/geo/languages` pages (nav `ready` flipped); `AudienceReport` renders an upgrade `EmptyState` on a 402 (city, free plan); new **`LiveCountries`** panel (emoji flags + live counts + indigo share bars) on the live page from `useLiveTraffic`'s new `currentCountries` (aggregates the live feed's existing `country`, no backend change). **Live map is shipped as a country panel, not a geographic choropleth** — a real map needs a bundled world-map/topojson asset (obtainable only online); deferred. **No new dep, no new env var** (`GEOIP_DB_PATH` already existed; city works only when the .mmdb is present, fails open otherwise). 258 backend tests pass (new `test_ch_migrations.py`, `test_geo.py`; gate tests in `test_stats_router.py`/`test_billing_service.py`; language passthrough in `test_ingest.py`), ruff clean; all new SQL + the full write→read path (insert `city`/`language` → read back) live-validated against real ClickHouse; web lint + tsc + build green (35 routes: +`/geo/cities`, +`/geo/languages`), 9 Vitest pass, grep gate holds. Manual: run `--migrate` in prod before deploying the new tracker/ingest; a real GeoLite2-City `.mmdb` at `GEOIP_DB_PATH` is required for city/region data. **Phase 10 (query-layer reports) built** — richer breakdowns + drill-down + dashboard-wide filtering, all on existing `events` data (no schema/tracker change). **Filtering foundation:** allowlisted `_FILTER_COLUMNS` (country/device/browser/os/source/path) threaded through every `services/stats.py` builder **and** inside `_SESSIONIZED_CTE` (so sessions/bounce/entry/exit honor filters); values bound as **namespaced server-side params** `{f_*:String}` — never string-formatted (§9), contract pinned in `test_stats_builders`. Shared `core/statsfilters.py::FilterDep` wired into `routers/stats.py` + `routers/public.py`. Frontend: in-memory `components/layout/filter-context.tsx` (`FilterProvider` in the dashboard layout), `components/dashboard/active-filters.tsx` chip bar in `ReportShell`, `onSelect` click-to-filter on Breakdown/Pages cards → every chart/table re-filters (this also **is** the page-detail drill-down: a page row sets a `path` filter). **New reports:** `services/channels.py` (direct/search/social/**AI**/referral — host lists shared by pure `classify()` + a SQL `multiIf`, AI matched before search; `/stats/channels` + `/stats/channels/{channel}` drill-down → Channels/Search/Social/AI sidebar pages, nav `ready` flipped); screen-width buckets on `/stats/audience?dimension=screen` (`/tech/screens`); time-of-day heatmap `/stats/heatmap` — **§4 exception: aggregated in the viewer's IANA tz** (`toHour(ts,{tz})`, `toDayOfWeek(ts,0,{tz})` — dow's tz is the 3rd arg, 2nd is week_mode; **live-E2E caught this ClickHouse type error unit tests missed**), 7×24 zero-filled grid → `components/reports/heatmap-card.tsx` in Overview; engagement page ranking (`sort=engagement`: per-page bounce % + avg time-on-page via `leadInFrame` over the session CTE; traffic/engagement toggle). **Custom date range:** native date-input popover (`components/dashboard/custom-range.tsx`, **dependency-free — no shadcn Calendar/react-day-picker**) → custom `[from,to)` in `range-context.tsx` (server already parses arbitrary ranges). **Full parity (decided):** filters + channels/screens/heatmap datasets flow into the public share router (`/public/{token}/…`) and CSV export (`services/export.py`). New schemas `ChannelRow`/`ChannelsOut`/`HeatmapCell`/`HeatmapOut` + `PageRow.avg_duration`/`bounce_rate`. **No new dep, no new env var, no migration.** 244 backend tests pass (new `test_channels.py` + Phase-10 builder tests; live-validated all new SQL against real ClickHouse), ruff clean; web lint + tsc + build green (33 routes: +acquisitions/{channels,search,social,ai}, +tech/screens), 9 Vitest pass (new `lib/range.test.ts`), grep gate holds. **Phase F7 (polish + support chatbot) built** (F-track, `frontendClaude.md`) — completes the F0–F7 frontend redesign track. New public `POST /assistant/chat` (`routers/assistant.py` + `services/assistant.py`): five hardcoded FAQ intents answered with no model call (keyless), unmatched → cheap `claude-haiku-4-5` fallback, else canned contact line. Guardrails: per-IP rate limit (30/hr), low max_tokens, single-message context, zero user/analytics-data access, system prompt forbids inventing pricing. New dep **anthropic** (§5, official async SDK, lazy/key-gated); new env `ANTHROPIC_API_KEY` (§6, blank → FAQ+contact fallback). Frontend: floating `components/ChatWidget.tsx` + `hooks/useAssistant.ts` mounted on marketing pages, FAQ suggestion chips from `content/faq.ts`. Polish: site-creation success toast. 228 backend tests pass (new `test_assistant.py`), ruff clean; web lint + tsc + build green (25 routes), 7 Vitest pass, grep gate holds; `/assistant/chat` live-verified. Manual: set a real `ANTHROPIC_API_KEY` for the AI fallback (FAQ works without it) + the dual-theme/mobile/a11y visual pass. **Phase F6 (footer + static pages) built** (F-track, `frontendClaude.md`): the trust layer. New public `POST /contact` (`routers/contact.py` + `services/contact.py`): honeypot + per-IP rate limit + transactional `send_email` (dev-stub logs; no marketing gate; no new env var). Frontend: rich `components/marketing/footer.tsx` (columns + X/GitHub/LinkedIn currentColor SVG marks + "Powered by Flowly"), slim `components/layout/app-footer.tsx` in the dashboard shell, new `/terms` + `/about` + restyled `/privacy` on a shared `prose-page.tsx`, a `/contact` form (`hooks/useContact.ts`, off-screen honeypot, new `ui/textarea.tsx`), and a custom `app/not-found.tsx`. No new deps/env/migration. 222 backend tests pass (new `test_contact.py`), ruff clean; web lint + tsc + build green (27 routes), 7 Vitest pass, grep gate holds; live `POST /contact` → 204. **Phase F5 (billing UI) built** (F-track, `frontendClaude.md`): billing surfaces prepped for the Phase 14 metered model, all reading `lib/pricing.ts`. Billing page redesigned (usage meter + month-to-date graduated **bill estimate** + Customer-Portal link; flat Pro/Business plan cards dropped since Phase 14 removes them); new `components/PaywallModal.tsx` (metered pitch + reused F1 `PricingSlider`, CTA → interim flat Checkout, `dismissible` prop for Phase 14's non-escapable `locked` wall); `components/UsageBanner.tsx` reworked into warning / over-locked / paying-bill-estimate states; `UsageSummary.status` gained `"locked"` (UI renders it now, Phase 14 emits it); new `/pricing` marketing page reusing the F1 `Pricing` section + slider, nav repointed off the `/#pricing` anchor. No 402 lock enforcement yet (Phase 14), no new deps/env/backend change. Web lint + tsc + build green (25 routes), 7 Vitest pass, grep gate holds. **Phase F4 (dashboard redesign) built** (F-track, `frontendClaude.md`): the authed dashboard re-rendered premium on the F1 `components/reports/` kit (StatCards/TrafficChartCard/BreakdownCard/PagesCard/row-icon/LiveDot) fed by `hooks/useStats.ts` — no backend/data change. New `lib/range.ts` + `components/layout/range-context.tsx` (dashboard-wide 24h/7d/30d range) + `components/dashboard/{report-shell,overview,report-views}.tsx`. Each Phase-5-backed report is now a real sidebar destination (`lib/navigation.ts` `ready:true` for Behavior Pages/Entry/Exit, Acquisitions Referrers/Campaigns, Geographic Countries, Technology Browsers/OS/Devices; Channels/Search/Social/AI/Cities/Languages/Screens stay off pending Phase 10/11) with thin route pages; Overview rebuilt with "View all" routing + Export CSV; live page gets the pulsing `LiveDot` + feed-in animation; old table-based `components/stats.tsx` deleted. No new deps/env/migration. Web lint + tsc + build green (24 routes), 7 Vitest pass, grep gate holds. **Phase F3 (user profile / settings) built** (F-track, `frontendClaude.md`): new authed `/account` surface — `services/account.py` + `routers/account.py` (change-password, two-step change-email with new-address code re-verification via a new `"email_change"` Redis-code purpose, email-preferences toggle over the existing `email_opt_out` column, list linked OAuth identities, and delete-account which wipes each site's ClickHouse events via new `retention.build_delete_all_query`/`delete_all_for_site` + evicts the Redis `site:{site_id}` map via new `billing.uncache_site_account`, then deletes Postgres children in FK order and the account). `AccountOut` gained `email_opt_out` + derived `has_password` (via new `AccountOut.from_account`; `/auth/me` updated), new `IdentityOut`. Web: `app/(dashboard)/settings` + `hooks/useAccount.ts` + `components/settings/{change-email,delete-account}-dialog.tsx` + shadcn `components/ui/switch.tsx` (Radix Switch, no new dep) + Settings in the sidebar Workspace group and header user menu. No new migration, no new dep, no new env var. 218 backend tests pass (new `test_account.py`), ruff clean; web lint + tsc + build green (`/settings` added), 7 Vitest tests pass, grep gate holds. **Phase F2 (auth pages / register system) built** (F-track, `frontendClaude.md`): sign-in/sign-up/forgot-password/reset-password/OAuth-callback all redesigned onto a split-panel shell (`components/auth/auth-shell.tsx` — brand panel + glass `AuthCard` + segmented gradient `AuthTabs`), **pixel-faithful to `desgin/demo-registersystem.png` per user call: the auth pages are a FIXED dark-violet cosmic scene in both themes** (the one deliberate dual-theme exception; no theme toggle there) — all scene colors scoped in `globals.css` `.auth-scene` (re-maps the F0 token vars, so components stay hex-free); scene art = the reference's own 3D renders cropped into `public/auth/{cube,gem,corner}.png`, screen-blended + mask-faded via `.auth-art` (single-layer masks only — Chromium mishandled multi-layer `mask-composite: intersect`), plus pure-CSS stars/planet/orbit/sphere, with reduced-motion-gated float/glow-pulse/twinkle keyframes — with icon-inset fields + inline validation (`components/auth/fields.tsx`, replaces the deleted `components/form.tsx`; `(dashboard)/sites` migrated too), restyled `SocialButtons` (Google brand-color SVG — brand assets exempt from the no-raw-hex rule), and post-auth routing `lib/post-auth.ts::postAuthPath()` (zero-site accounts → `/sites`, else `/dashboard`, fail-open) wired into password login + OAuth callback. No new deps/env vars/backend changes. Web lint + tsc + build green, 7 Vitest tests pass, grep gate holds. Also fixed a hydration mismatch in `components/motion.tsx`: motion's `useReducedMotion` reads matchMedia on the first client render (mismatches SSR when the OS prefers reduced motion, shifting every downstream `useId`) — replaced with a `useSyncExternalStore` hook whose server snapshot is `false`. **Phase F1 (landing page) built** (F-track, `frontendClaude.md`): new `app/(marketing)/` route group (nav + placeholder footer; `/` and `/privacy` moved in, URLs unchanged) with the one-scroll landing page — sticky nav, staggered hero with ONE indigo headline word, claims strip (honest social-proof substitute), 6-card features grid, how-it-works, privacy USP, **metered pricing section** (free-vs-paying cards + graduated rate table + snap-point slider, all reading `lib/pricing.ts` — **the ONE source of pricing truth**, integer-cents graduated math with boundaries pinned by `lib/pricing.test.ts` under a new **minimal Vitest setup** (`pnpm --filter web test` now real)), FAQ (shadcn Accordion over single-source `content/faq.ts` — F7's chatbot reuses it), final CTA. **Hero embeds a live demo dashboard** (`components/public-dashboard/demo-embed.tsx`): Flowly's own stats via the Phase 8 public share API, token from new `NEXT_PUBLIC_DEMO_SHARE_TOKEN` (+ previously-undocumented `NEXT_PUBLIC_API_URL` now in `.env.example`/§6; real values in `apps/web/.env.local`); unset/failed token → same composition fed labeled sample data (`components/reports/sample-data.ts`). **Share page restyled to the phpAnalytics reference** via a new presentational kit `components/reports/` (delta StatCards + CountUp, TrafficChartCard, icon-rich Breakdown/Pages cards with indigo share bars + "View all", `row-icon.tsx` DDG-favicon/emoji-flag/lucide icons, LiveDot) composed by `components/public-dashboard/{public-dashboard,report-sidebar}.tsx` (grouped in-page-state sidebar scoped to the public API) — hero embed + share page are compositions of the SAME kit; F4 reuses it for the authed dashboard (`components/stats.tsx` remains until then). **Motion**: new dep `motion` (§5 Animation row) via `components/motion.tsx` (Reveal/Stagger/CountUp — reduced-motion gated, ≤500ms, CountUp animates ref textContent to satisfy the set-state-in-effect lint rule); hover lifts + live pulse are pure CSS. New shadcn: accordion, slider. Web lint + tsc + build green (routes unchanged), 7 Vitest pricing tests pass, grep gate holds. Manual steps left: dual-theme/reduced-motion/mobile visual pass and minting a real demo share token for production. **Phase F0 (design system foundation) built** (first phase of the frontend redesign track F0–F7, tracked in `frontendClaude.md`): design tokens live in `apps/web/app/globals.css` `@theme inline` (**Tailwind v4 CSS-first — no `tailwind.config.ts`**, dark mode = `.dark` class via `@custom-variant dark`) — indigo primary `#6366F1` (+`--primary-hover`), slate neutrals, semantic `--success`/`--warning`/`--destructive` (green/red reserved for live/up/down/error), `--chart-1..5`; **next-themes** (system default, `ThemeProvider` in `providers.tsx`, `suppressHydrationWarning` on `<html>`, `components/theme-toggle.tsx` with CSS-swapped icons); **shadcn/ui** kit copied into `components/ui/` (new CLI `init -b radix -p nova`; Button/Card/Input/Select/Table/Dialog/Tabs/Badge/Skeleton/DropdownMenu/Sheet + **sonner as Toast** + custom `EmptyState`; Button default hover → `bg-primary-hover`); duplicated pill-Tabs/site-`<select>`s/"Loading…" strings killed → shared `components/segmented-tabs.tsx` (thin adapter over shadcn Tabs), `SiteProvider`/`useActiveSite()` (`components/layout/site-context.tsx`, localStorage-persisted via `useSyncExternalStore` — the `react-hooks/set-state-in-effect` lint rule forbids setState-in-effect), `components/skeletons.tsx`; **app shell** in `(dashboard)/layout.tsx` (auth guard unchanged): `components/layout/{app-sidebar,app-header,site-switcher}.tsx` driven by `lib/navigation.ts` — the FULL future sidebar IA (Behavior/Acquisitions incl. AI platforms/Geographic/Technology) as data with `ready` flags, only shipped pages render; header = mobile Sheet nav + SiteSwitcher + date-range-picker slot (F1) + theme toggle + user menu; **Recharts theme module** `components/charts/chart-theme.tsx` (colors as `var(--chart-N)` straight into SVG props → auto theme flip; `ChartTooltip`/`ChartGradient`/grid/axis presets; TrafficChart = indigo Area + gradient, compact axes); formatters centralized in `lib/format.ts`; the Geist-defeating `font-family: Arial` bug in globals.css fixed. New deps: next-themes, shadcn (runtime), radix-ui, cva, clsx, tailwind-merge, lucide-react, tw-animate-css, sonner (§5 updated). Grep gate: no raw hex/`gray-*`/`bg-black` utilities outside `globals.css` (exception: vendored dialog/sheet scrim). Web lint + tsc + build green (13 routes). Manual dual-theme visual pass is the remaining verification step. **tz-param fix:** ClickHouse `DateTime` params are now bound **tz-aware UTC** (`services/stats.py::_range_params`, `services/retention.py::build_delete_query`); a naive param is rendered as a bare string that ClickHouse parses in the *server* timezone, which shifted every stats window and retention cutoff by the server's UTC offset — on a BST dev server the newest hour of events was invisible in every "until now" report (dashboard, public share, CSV export, digest). Found by live E2E (unit tests don't run real ClickHouse); contract pinned in `test_stats_builders`/`test_retention`. 206 backend tests pass, ruff clean. **Roadmap (2026-07-04):** new Phases 10–14 added (query-layer reports & filtering; city/language/live-map + first ClickHouse migration convention; uptime monitoring & down alerts; Search Console keywords/SEO; **Pricing v2 — graduated metered billing REPLACING the flat plans: 1k free / $0.99 per 1k to 10k / $0.10 per 1k to 100k / $0.05 per 1k to 1M / $0.03 beyond, views summed account-wide across all (max 5) sites, dashboard paywall modal when a free account passes 1k whose Checkout carries the repositioned 7-day trial — trial starts at upgrade, once per account; ingestion never blocked, §9**); old deferred Phase 10 renumbered to **15** (Search Console removed from it — now Phase 13) and a **Parked** list records what we decided against (new-vs-returning, 404 tracking, CWV/JS errors, paid keyword research). Phases 0–9 untouched. Build note history: Phase 8 (growth & retention) + Phase 9 (privacy & trust) built. **Shared email prereq:** `services/email.py` now delivers via **Resend** (a single httpx POST in `_send_via_provider`; still dev-stub-logs in local/no-key) with an optional HTML body; new `services/notifications.py` is the **marketing gate** — `send_marketing_email` refuses opted-out accounts and appends a signed one-click unsubscribe footer, `marketing_recipients` lists verified+not-opted-out accounts, `apply_unsubscribe` flips the flag; `accounts.email_opt_out` column + `create_unsubscribe_token` (new `unsubscribe` JWT type, 10y) + public `routers/email.py` `GET /email/unsubscribe`. Transactional mail (verify/reset, trial-ending) never routes through the gate. **8.1 digest:** `services/digest.py` builds a per-site trailing-week summary by reusing the Phase 5 stats services (no bespoke SQL) → `render_digest` (subject/html/text); `workers/digest.py` (run-once, cron weekly) iterates recipients, skips zero-traffic, sends via the gate, and marks `digest:{account_id}:{ISOyearweek}` in Redis (10-day TTL) for idempotency. **8.2 share + 8.3 badge:** `models.tables.ShareToken` (secret `token_urlsafe`, FK to site pk, soft `revoked_at`) + `services/sharing.py` (create=rotate revokes prior, `resolve_share` only returns live tokens, single-site scope); authed create/rotate/revoke on `routers/sites.py` (`/sites/{id}/share`), **public** `routers/public.py` (`/public/{token}/…`) reuses the stats services with a token gate replacing bearer auth (404 on unknown/revoked before any query) — `GET /public/{token}` returns `{domain, show_badge}` where `show_badge = effective_plan==free`; web `app/share/[token]` read-only dashboard (`hooks/usePublicStats.ts`, unauth `publicFetch`) + `components/{PoweredByBadge,ShareControl}.tsx`. Shared `core/timerange.py::stats_range` extracted from `routers/stats.py` so both stats + public routers use one window parser. **8.4 onboarding:** `models.tables.OnboardingEmail` ((account,step) unique ledger) + `services/onboarding.py` (welcome/install/live content + `already_sent`/`record_step`); `workers/onboarding.py` (run-once, cron hourly) advances each account — welcome once, `live` on first event (`sites.first_event_seen`), `install` nudge only if dark after 24h (and marked-without-emailing once live). `trial_will_end` wired via `billing.on_event_committed` (post-commit, best-effort transactional email) called from the webhook router. **9.1 retention:** `config.RETENTION_DAYS` (free 30 / pro 365 / business 730) + `services/retention.py` (pure `(sql,params)` site-scoped `ALTER TABLE events DELETE WHERE site_id AND ts<cutoff`, cutoff from `effective_plan` so a lapsed trial ages out at 30d) + `db/clickhouse.py::run_command`; `workers/retention.py` (run-once, cron daily) sweeps every site, logs each cutoff (destructive by design, §7). `events` was already `PARTITION BY toYYYYMM(ts)` so no CH schema change. **9.2 export:** `services/export.py` (aggregated reports only — never raw events, no `visitor_hash`/IP) + `GET /stats/export` (authed, `owned_site`+`stats_range`, `text/csv` download). **9.3 privacy:** static Server Component `app/privacy` + landing-page link. **billing hardening:** `effective_plan`/onboarding now coerce a naive stored `trial_ends_at`/`created_at` to UTC (§4) so a mixed-awareness read can't raise (prod asyncpg is already aware; guards the SQLite test path). Migration `f3a9c1d47b20` (batch-mode): `email_opt_out` + `share_tokens` + `onboarding_emails`; applied + downgrade-verified on dev Postgres, single head. **No new deps** (Resend rides existing httpx), **no new env var** (`EMAIL_API_KEY`/`EMAIL_FROM` already present). §7 gained the three worker cron commands. 206 backend tests pass (new `test_notifications`/`test_sharing`/`test_public_router`/`test_digest`/`test_onboarding`/`test_retention`/`test_export`/`test_billing_trial`, extended `test_migration`); web lint + tsc + build green (new `/privacy`, `/share/[token]` routes). Live Resend + Stripe smoke and wiring the three workers to a real scheduler are the manual steps left. Phase 7 post-review hardening: (1) removed the dead `resolve_account_id` (nothing called it; its "self-heals from Postgres" promise was never wired) — the site→account map is now honestly documented as write-once at `create_site`, and a lost best-effort write just under-counts that site until re-saved (never blocks ingestion); `meter_pageview` now debug-logs a cold miss. (2) `create_checkout_session` reuses a known `stripe_customer_id` and **refuses** checkout for an already active/past_due account (402 → use the portal), closing a duplicate-customer / double-subscription hole; it now takes a session. (3) webhook `_account_from_event_object` guards the `UUID(metadata)` parse so a malformed id can't 500-loop Stripe retries. (4) `/script.js` returns 404 (not 500) when the tracker isn't built. Ruff check+format clean; 167 backend tests pass (added checkout-guard + Redis-only cache round-trip tests). Phase 7 build note follows. Phase 7 (billing & usage metering) built: trial converts to paid, metered by pageviews. **Split of truth:** Stripe = entitlement, Redis = usage, Postgres = durable mirror. `services/billing.py` holds the metering half (`record_usage`/`meter_pageview` → `INCR usage:{account_id}:{YYYYMM}` + 45-day TTL; Redis-only `cached_account_id` on the hot path so `/collect` stays off Postgres — the `site:{site_id}→account_id` map is written by `create_site`; `resolve_account_id` warms it from Postgres on a cold miss) plus read-time entitlement: `effective_plan` derives the tier (active/past_due/in-window-trial → `account.plan`, else `free`), so a **lapsed card-free trial or canceled sub downgrades to `free` with no webhook/job**; `usage_summary` → `{plan,quota,used,pct,status: ok|warning(≥80%)|over(≥100%)}`; `over_hard_ceiling` (3× quota) is a runaway guard, **not** enforced on the hot path (would need a Postgres read per event — burst cost is bounded by the Phase 3 rate limit instead). Soft cap: data is **never dropped at the quota** (§9). Stripe half: `create_checkout_session` (mode=subscription, `trial_end=trial_ends_at` pass-through so no double-trial/early-charge, account id in `client_reference_id` + subscription metadata), `create_portal_session` (402 `BillingError` if no customer), `verify_webhook` (sync signature check), and `apply_subscription_event` — **the ONLY writer of entitlement** (never the redirect, §10): `subscription.created|updated` → set `account.plan/status` + upsert `subscriptions` (price/customer/sub id, `cancel_at_period_end`, `current_period_end`), `deleted` → `free`/`canceled`, `invoice.payment_failed` → `past_due` (keeps serving), `trial_will_end` → nudge no-op; account resolved via subscription metadata (order-independent) or `stripe_customer_id`. `routers/billing.py` (thin): authed `POST /billing/checkout`, `POST /billing/portal`, `GET /billing/usage`; **public** `POST /billing/webhook` — verify sig (bad → 400 `WebhookSignatureError`) → dedupe via `processed_stripe_events` (event_id PK) → apply + insert id in one transaction (IntegrityError on a raced redelivery → idempotent ack). New `BillingError` (402) in `core/exceptions.py`; the webhook's bad-signature 400 is a router-local `WebhookSignatureError`. Metering hooks into `services/ingest.py` best-effort after the durable XADD (only counted events; bots/rate-limited already returned). Migration `c7f1b2f13d02` (batch-mode): `subscriptions.stripe_price_id` + `cancel_at_period_end`, new `processed_stripe_events` table + `ProcessedStripeEvent` model; applied + downgrade-verified on dev Postgres. `config.py` gained the Stripe block (secret/webhook/4 price ids) + `PLAN_QUOTAS` (`free 10k / pro 100k / business 1M`, placeholders) + `HARD_CEILING_MULTIPLE=3`. Frontend: `hooks/useBilling.ts` (`useUsage`/`useCheckout`/`usePortal`), `components/UsageBanner.tsx` (amber ≥80% / red ≥100% nudge, injected in the dashboard `layout.tsx` — the only shared chrome; never blocks), `app/(dashboard)/billing/page.tsx` (plan + usage bar, monthly/annual upgrade → Checkout, Customer Portal, `?checkout=success|cancel` return), and a "Billing" dashboard link. New dep **stripe** (15.3.0); new env vars `STRIPE_PRICE_PRO_ANNUAL`/`STRIPE_PRICE_BUSINESS_ANNUAL` (in `.env.example` + §6). 165 backend tests pass (`test_billing_service`, `test_billing_router`, extended `test_ingest`/`test_sites_service`/`test_migration`); web lint + tsc + build green. Live Stripe smoke (Checkout/Portal/webhook via `stripe listen`) is the one manual step left. Phase 6 hardening note follows. Phase 6 hardening (post-review): (1) **duplicate-domain race** closed — the `sites` table gained a `UniqueConstraint("account_id", "domain", name="uq_site_account_domain")` (migration `4962f38126db`, down-rev `0b5153c24ddf`, batch-mode so it runs on SQLite too), and `services/sites.py::create_site` now wraps its `commit()` in a `try/except IntegrityError → ConflictError` (mirrors `auth.py::signup`) so a lost race surfaces a clean 409, not a 500; the pre-check `SELECT` stays as the friendly fast path. (2) **`/sites` is now a manage screen** — `app/(dashboard)/sites/page.tsx` lists existing sites (each with an "Open" → reuses `InstallStep`, now with a "← All sites" back link) above the add form, keeping the app's URL-free in-page-state convention (no dynamic route); the dashboard header link is relabelled "Sites". Migration applied + downgrade-verified on dev Postgres (no existing dupes); 133 backend tests pass (new `test_duplicate_domain_race_caught_by_db_constraint`); web lint + tsc + build green. Phase 6 note follows. Phase 6 (site onboarding) built: a user can add a site, get an install snippet, and watch it flip waiting→connected. `core/urls.py::normalize_host` was extracted from `services/ingest.py::_host` (now imported by both ingest + sites) — it stays **non-raising** (fail-open, so `/collect` can't break) and now handles **bare-domain** input (`example.com`; `urlparse` needs a `//` prefix or its `netloc` is empty). `services/sites.py` gained `create_site` (normalize domain → `ValidationError` if empty; account-scoped duplicate-domain → `ConflictError`; `secrets.token_hex(8)` `site_id` with a 3-try collision retry backed by the `UNIQUE` index), `first_event_seen` (Redis `count_active` first for an instant signal, else a server-parameterized ClickHouse `SELECT 1 … LIMIT 1` existence probe — durable, no new column), `build_snippet` (from new `TRACKER_SCRIPT_URL` in `config.py`), and the canonical `to_site_out` (attaches the computed `snippet`, never `model_validate`). New `routers/sites.py` (`prefix="/sites"`): `GET /sites` (moved out of `live.py`), `POST /sites` (201), `GET /sites/{site_id}`, `GET /sites/{site_id}/status` — the per-site routes use a **path-param** `owned_site` dep (404 before any Redis/CH query, §9). `domain` is a cosmetic label (events scoped by `site_id`, not origin — enforcement deferred). Frontend: `hooks/useSites.ts` gained `useCreateSite` + `useSiteStatus` (polls, stops on connected and after a ~3-min cap + manual re-check), `components/install.tsx` (snippet copy-box + per-platform tabs + status pill), `app/(dashboard)/sites/page.tsx` (add-domain → install two-step), and "Add site" CTAs on the dashboard/live empty states. `SiteOut` gained a required `snippet`; `SiteCreate`/`SiteStatus` added. No new migration (schema unchanged), no new dep; one new env var (`TRACKER_SCRIPT_URL`, already in `.env.example`). 132 backend tests pass (incl. `test_urls.py`, `test_sites_service.py`, extended `test_sites.py`); web lint + tsc + build green. Phase 5 note follows. Phase 5 (historical dashboard metrics) built: `services/stats.py` holds pure `(sql, params)` query-builders + a shaping service — overview (visitors/sessions/pageviews/bounce/avg-duration) with prior-period compare, time-series (hour/day buckets auto-picked from range length, gaps zero-filled in Python), sources+UTM, audience (country/device/browser/os), and pages (top/entry/exit). Sessions/bounce/duration are derived in SQL from a 30-min `visitor_hash`+`ts` gap via `_SESSIONIZED_CTE` (`lagInFrame` → session starts → running sum); there is no `session` column. All ClickHouse SQL is **server-parameterized** (`{site_id:String}`) — no user value string-formatted in (§9). `db/clickhouse.py` gained a raw `query_rows` read helper. `routers/stats.py` exposes `/stats/{overview,timeseries,sources,audience,pages}`, each gated by the `owned_site` dependency that verifies ownership (→ 404) **before** any query. Ownership (`get_owned_site`/`list_account_sites`) was extracted from `services/live.py` into shared `services/sites.py` (live + tests updated). Frontend: `hooks/useStats.ts` (TanStack Query, keyed by endpoint+site+range), `components/stats.tsx` (metric cards + Recharts line chart + sources/audience/pages/UTM tables), and `app/(dashboard)/dashboard/page.tsx` (site picker, 24h/7d/30d presets, audience/pages tabs). New dep: **recharts** (already in §5). No new env var (`SESSION_TIMEOUT_SECONDS=1800`, `MAX_RANGE_DAYS=372` are constants). Cookieless caveat: daily `visitor_hash` rotation over-counts multi-day unique visitors (accepted tradeoff). 102 backend tests pass; web lint + tsc + build green. Phase 4 (live) note follows. Phase 4 (real-time live traffic) built: `services/live.py` holds the presence ZSET + pub/sub — `mark_active` pipelines `ZADD active:{site_id}` + stale eviction + key `EXPIRE` (bounded with no viewer), `count_active` evicts-then-`ZCARD`, `publish_event` fans to `live:{site_id}`, and `subscribe_events` owns a per-connection pub/sub (with an `on_ready` hook so the WS subscribes before sending its count snapshot). `services/ingest.py` calls `live.record_and_publish` (presence + publish in one pipeline) best-effort right after the `XADD` (never blocks/fails `/collect`; forwarded payload is IP-free and hash-free). `routers/live.py` serves `WS /live/{site_id}` — accept → Origin check → `?token=` verify (`decode_token`) → site-ownership on a short-lived session → subscribe → snapshot → forward/heartbeat/receiver tasks, all rejections `close(1008)` — plus a read-only ownership-scoped `GET /sites` (thin borrow from Phase 6) and the `SiteOut` schema. Frontend: `hooks/useLiveTraffic.ts` (native WebSocket, capped feed, backoff reconnect, one token-refresh retry on 1008), `hooks/useSites.ts`, `components/live.tsx`, and `app/(dashboard)/live/page.tsx` (auto-guarded, site picker, remount-per-site via key). No new dep, no new env var (`LIVE_WINDOW_SECONDS=300` is a constant). 72 tests pass. Phase 3 (ingestion), Phase 2 (tracker), Phase 1 (auth) previously built. Keep this date current whenever the stack or rules change._
