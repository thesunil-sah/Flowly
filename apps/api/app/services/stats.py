...
""".replace("{base_where}", _BASE_WHERE).replace("__TIMEOUT__", str(SESSION_TIMEOUT_SECONDS))
...
def build_overview(site_id: str, from_: datetime, to: datetime) -> tuple[str, dict[str, Any]]:
    ...
    return sql, _range_params(site_id, from_, to)


def build_timeseries(
    site_id: str, from_: datetime, to: datetime, bucket_fn: str
    ...
    sql = (
        ...
        "toInt64(uniqExact(visitor_hash)) AS visitors "
        "FROM events WHERE " + _BASE_WHERE + " GROUP BY bucket ORDER BY bucket"
    )
    return sql, _range_params(site_id, from_, to)


def build_breakdown(
    site_id: str, from_: datetime, to: datetime, column: str, limit: int
) -> tuple[str, dict[str, Any]]:
    ...
    sql = (
        ...
        "toInt64(count()) AS pageviews, "
        "toInt64(uniqExact(visitor_hash)) AS visitors "
        "FROM events WHERE " + _BASE_WHERE + " GROUP BY label "
        "ORDER BY visitors DESC, pageviews DESC, label ASC LIMIT {limit:UInt32}"
    )
    params = _range_params(site_id, from_, to) | {"limit": _clamp_limit(limit)}
    return sql, params


def build_utm(
    site_id: str, from_: datetime, to: datetime, limit: int
) -> tuple[str, dict[str, Any]]:
    ...
    sql = (
        ...
        "toInt64(count()) AS pageviews, "
        "toInt64(uniqExact(visitor_hash)) AS visitors "
        "FROM events WHERE " + _BASE_WHERE + " AND utm_source != '' "
...
def build_pages(
    site_id: str, from_: datetime, to: datetime, kind: str, limit: int
) -> tuple[str, dict[str, Any]]:
    ...
    if kind == "top":
        sql = (
            ...
            "toInt64(uniqExact(visitor_hash)) AS visitors "
            "FROM events WHERE " + _BASE_WHERE + " GROUP BY label" + limit_clause
        )
    elif kind == "entry":
        # The first event of each session is flagged is_start.
        sql = _SESSIONIZED_CTE + (
            " SELECT path AS label, toInt64(count()) AS count, "
            "toInt64(uniqExact(visitor_hash)) AS visitors "
...
async def overview(
    client: AsyncClient, site_id: str, from_: datetime, to: datetime, compare: bool
) -> OverviewOut:
    sql, params = build_overview(site_id, from_, to)
    current = _metrics_from_row((await query_rows(client, sql, params))[0])

    previous: dict[str, float] | None = None
    if compare:
        pfrom, pto = _previous_window(from_, to)
        psql, pparams = build_overview(site_id, pfrom, pto)
...
async def timeseries(
    client: AsyncClient, site_id: str, from_: datetime, to: datetime
) -> TimeseriesOut:
    bucket_fn, is_hour = _pick_bucket(from_, to)
    sql, params = build_timeseries(site_id, from_, to, bucket_fn)
...
async def audience(
    client: AsyncClient, site_id: str, from_: datetime, to: datetime, dimension: str, limit: int
) -> BreakdownOut:
    column = _AUDIENCE_COLUMNS.get(dimension)
    if column is None:
        raise ValidationError(f"Unknown dimension: {dimension}")
    sql, params = build_breakdown(site_id, from_, to, column, limit)
    return BreakdownOut(
...
async def sources(
    client: AsyncClient, site_id: str, from_: datetime, to: datetime, limit: int
) -> SourcesOut:
    ssql, sparams = build_breakdown(site_id, from_, to, "source", limit)
    usql, uparams = build_utm(site_id, from_, to, limit)
...
async def pages(
    client: AsyncClient, site_id: str, from_: datetime, to: datetime, kind: str, limit: int
) -> PagesOut:
    if kind not in _PAGE_KINDS:
        raise ValidationError(f"Unknown page kind: {kind}")
    sql, params = build_pages(site_id, from_, to, kind, limit)
    rows = [
        PageRow(label=str(r["label"]), count=int(r["count"]), visitors=int(r["visitors"]))
        for r in await query_rows(client, sql, params)
    ]
    # `top` counts pageviews; entry/exit count sessions landing/leaving on a path.
    metric = "pageviews" if kind == "top" else "sessions"
